...
from models.vrwkv.utils import DropPath
...
def resize_pos_embed(pos_embed, src_shape, dst_shape, mode='bicubic'):
    ...
    resized = F.interpolate(pos_embed, size=(dst_H, dst_W), mode=mode)
    resized = resized.permute(0, 2, 3, 1).flatten(1, 2)
    return resized
...
if __name__ == '__main__':
    ...
    batch_size = 2
    x = torch.randn(batch_size, 3, 224, 224).cuda()  # [B, C, H, W]
    outputs = model(x)

    print("\nModel outputs:")
    for i, out in enumerate(outputs):
        print(f"Output {i + 1} shape: {out.shape}")  # 应该为 [batch_size, 256, 14, 14]
